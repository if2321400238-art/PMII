                        ...
                        @forelse($posts as $post)
                            <tr class="hover:bg-gray-50 transition">
                                <td class="px-3 md:px-6 py-2 md:py-4">
                                    <p class="font-semibold text-gray-900 line-clamp-1 text-sm md:text-base">{{ $post->title }}</p>
                                    <p class="text-xs text-gray-500 mt-1">{{ $post->slug }}</p>
                                </td>
                                <td class="px-3 md:px-6 py-2 md:py-4 hidden sm:table-cell">
                                    <span class="px-2 md:px-3 py-1 rounded-full text-xs font-semibold {{ $post->type === 'berita' ? 'bg-blue-100 text-blue-800' : 'bg-purple-100 text-purple-800' }}">
                                        {{ ucfirst($post->type) }}
                                    </span>
                                </td>
                                <td class="px-3 md:px-6 py-2 md:py-4 hidden md:table-cell text-xs md:text-sm">{{ $post->category->name }}</td>
                                <td class="px-3 md:px-6 py-2 md:py-4 hidden lg:table-cell text-xs md:text-sm">{{ $post->author->name }}</td>
                                    ...
                                    <div class="flex gap-1 justify-center flex-wrap">
                                        ...
                                        @can('approve', $post)
                                            @if($post->approval_status === 'pending')
                                                ...
                                                <button onclick="showRejectForm({{ $post->id }})" class="px-2 md:px-3 py-1 text-xs md:text-sm bg-red-600 text-white rounded hover:bg-red-700 whitespace-nowrap">
                                                    ✗ Tolak
                                                </button>
