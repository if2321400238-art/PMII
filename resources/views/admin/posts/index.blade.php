    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="block md:hidden space-y-4">
            @forelse($posts as $post)
                <article class="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
                    <div class="flex justify-between items-start mb-2">
                        <div class="flex-1 min-w-0 pr-2">
                            <h3 class="font-semibold text-gray-900 text-base line-clamp-2">{{ $post->title }}</h3>
                            <p class="text-xs text-gray-500 mt-1 truncate">{{ $post->slug }}</p>
                        </div>
                        <span class="flex-shrink-0 px-2 py-1 rounded-full text-xs font-semibold {{ $post->type === 'berita' ? 'bg-blue-100 text-blue-800' : 'bg-purple-100 text-purple-800' }}">
                            {{ ucfirst($post->type) }}
                        </span>
                    </div>
                    ...
                    <div class="flex items-center justify-between mb-3">
                        ...
                        @elseif($post->approval_status === 'draft')
                    ...
                    <div class="flex flex-wrap gap-2 pt-3 border-t border-gray-100">
                        @can('approve', $post)
                            @if($post->approval_status === 'pending')
                                <form method="POST" action="{{ route('admin.posts.approve', $post) }}" class="inline-block">
                                    @csrf
                                    <button type="submit" class="px-3 py-2 text-xs font-medium bg-green-600 text-white rounded-lg hover:bg-green-700">
                                        ✓ Setujui
                                    </button>
                                </form>
                                <button onclick="showRejectForm({{ $post->id }})" class="px-3 py-2 text-xs font-medium bg-red-600 text-white rounded-lg hover:bg-red-700">
                                    ✗ Tolak
                                </button>
                            @endif
                        @endcan
                        ...
                        @endcan
                        @can('delete', $post)
                            <form method="POST" action="{{ route('admin.posts.destroy', $post) }}" class="flex-1" onsubmit="return confirm('Yakin hapus post ini?')">
                                @csrf
                                @method('DELETE')
                ...
                @can('approve', $post)
                    @if($post->approval_status === 'pending')
                        <div id="reject-form-{{ $post->id }}" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                            <div class="bg-white p-4 md:p-6 rounded-lg max-w-md w-full">
                                ...
                                <form method="POST" action="{{ route('admin.posts.reject', $post) }}">
                                    ...
                                    <div class="flex gap-2">
                                        ...
                                        <button type="button" onclick="hideRejectForm({{ $post->id }})" class="px-4 py-2 bg-gray-300 rounded hover:bg-gray-400 text-sm font-semibold">
                                            Batal
                                        </button>
                                    </div>
                                </form>
                        ...
                        @forelse($posts as $post)
                            <tr class="hover:bg-gray-50 transition">
                                <td class="px-6 py-4">
                                    ...
                                    <p class="text-xs text-gray-500 mt-1">{{ $post->slug }}</p>
                                </td>
                                <td class="px-6 py-4">
                                    <span class="px-3 py-1 rounded-full text-xs font-semibold {{ $post->type === 'berita' ? 'bg-blue-100 text-blue-800' : 'bg-purple-100 text-purple-800' }}">
                                        {{ ucfirst($post->type) }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 text-sm">{{ $post->category->name }}</td>
                                <td class="px-6 py-4 text-sm">{{ $post->author?->name ?? 'Unknown' }}</td>
                                    ...
                                    <div class="flex gap-2 justify-center flex-wrap">
                                        @can('approve', $post)
                                            @if($post->approval_status === 'pending')
                                                ...
                                                <button onclick="showRejectForm({{ $post->id }})" class="px-3 py-1 text-xs bg-red-600 text-white rounded hover:bg-red-700">
                                                    ✗ Tolak
                                                </button>
                                        ...
                                        @can('delete', $post)
                                            <form method="POST" action="{{ route('admin.posts.destroy', $post) }}" class="inline-block" onsubmit="return confirm('Yakin hapus post ini?')">
                                                @csrf
                                                @method('DELETE')
