...
<div class="container mx-auto px-4 py-4 md:py-8">
    <div class="max-w-7xl mx-auto">
        ...
        <div class="mb-6 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
            <div>
                <h1 class="text-2xl md:text-3xl font-bold text-gray-900">Kelola Korwil</h1>
                <p class="text-gray-600 text-sm md:text-base mt-1">Kelola data koordinator wilayah organisasi</p>
            </div>
            <a href="{{ route('admin.korwil.create') }}"
               class="px-4 md:px-6 py-2.5 md:py-3 bg-gradient-to-r from-green-600 to-green-700 text-white rounded-lg hover:from-green-700 hover:to-green-800 transition font-medium shadow-lg shadow-green-500/30 flex items-center justify-center text-sm md:text-base w-full sm:w-auto">
        ...
        <!-- Mobile Cards View -->
        <h2 class="sr-only">Daftar Korwil</h2>
        <div class="block md:hidden space-y-4">
            @forelse($korwils as $korwil)
                <article class="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
                    <div class="flex justify-between items-start mb-3">
                        <div class="flex-1 min-w-0">
                            <h3 class="font-semibold text-gray-900 text-base truncate">{{ $korwil->name }}</h3>
                            <p class="text-sm text-gray-600 mt-1">{{ $korwil->wilayah }}</p>
                        </div>
                        <span class="ml-2 px-2.5 py-1 text-xs font-medium rounded-full bg-blue-100 text-blue-700 border border-blue-200 flex-shrink-0">
                    ...
                    <div class="flex gap-2 pt-3 border-t border-gray-100">
                        ...
                        <form method="POST" action="{{ route('admin.korwil.destroy', $korwil) }}"
                              onsubmit="return confirm('Apakah Anda yakin ingin menghapus korwil ini?')" class="flex-1">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
            ...
            @empty
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-8 text-center">
                    ...
                    <p class="text-gray-500 text-sm mt-1">Tambahkan korwil pertama untuk memulai</p>
                </div>
            @endforelse
        </div>
