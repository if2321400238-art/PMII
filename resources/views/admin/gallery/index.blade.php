    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4 mb-6">
            <form method="GET" class="flex gap-3 items-center flex-wrap">
                ...
                @if(request()->has('type') || request()->has('approval_status'))
                    <a href="{{ route('admin.gallery.index') }}" class="text-sm text-blue-600 hover:underline">Reset Filter</a>
                @endif
            </form>
        </div>
                        ...
                        @forelse($galleries as $gallery)
                            <tr class="hover:bg-gray-50 transition">
                                <td class="px-6 py-4">
                                    ...
                                    <div class="text-xs text-gray-500 mt-1">{{ Str::limit($gallery->description, 80) }}</div>
                                </td>
                                <td class="px-6 py-4">
                                    <span class="px-3 py-1 text-xs font-semibold rounded-full {{ $gallery->type === 'photo' ? 'bg-blue-100 text-blue-800' : 'bg-purple-100 text-purple-800' }}">
                                        {{ ucfirst($gallery->type) }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 text-sm text-gray-700">{{ $gallery->kegiatan ?? '-' }}</td>
                                <td class="px-6 py-4 text-sm text-gray-700">{{ $gallery->tahun ?? '-' }}</td>
                                <td class="px-6 py-4">
                                    @if($gallery->approval_status === 'approved')
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex flex-col gap-2">
                                        ...
                                        @if(in_array($role, ['admin']) && $gallery->approval_status === 'pending')
                                            ...
                                            <!-- Form Reject (Hidden) -->
                                            <div id="reject-form-gallery-{{ $gallery->id }}" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                                        ...
                                        <div class="flex gap-2 justify-center">
                                            ...
                                            <form action="{{ route('admin.gallery.destroy', $gallery) }}" method="POST" onsubmit="return confirm('Hapus item ini?')" class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
                                                        class="inline-flex items-center px-3 py-1.5 text-xs font-medium bg-red-600 text-white rounded-lg hover:bg-red-700 transition">
