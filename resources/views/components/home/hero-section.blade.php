                        ...
                        <div class="absolute inset-0 p-4 flex flex-col justify-between z-20">
                            ...
                            <div class="hero-mobile-content p-4 rounded-2xl bg-gradient-to-t from-[#071a35]/55 via-[#071a35]/28 to-transparent">
                                ...
                                <div class="flex gap-2">
                                    <a href="{{ route('posts.berita') }}" data-magnetic class="magnetic-hover relative overflow-hidden inline-flex min-h-11 items-center justify-center rounded-full bg-yellow-400 px-4 text-sm font-bold text-[#0f172a] hover:bg-yellow-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-yellow-300 transition">
                                        <span class="cta-shimmer absolute inset-0 pointer-events-none"></span>
                                        <span class="relative z-10">Baca Berita</span>
                                    </a>
                                    <a href="{{ route('about.profil') }}" data-magnetic class="magnetic-hover inline-flex min-h-11 items-center justify-center rounded-full border border-white/60 bg-white/15 px-4 text-sm font-semibold text-white hover:bg-white/25 focus:outline-none focus-visible:ring-2 focus-visible:ring-white transition">
                                        Tentang PMII
                                    </a>
                                </div>
                            </div>
                    ...
                    <div class="rounded-2xl border border-white/15 bg-[#16355a]/80 backdrop-blur-sm p-2.5 shadow-xl shadow-black/25">
                        <div class="grid grid-cols-2 gap-2 mb-2">
                            <div class="rounded-xl bg-[#0b1f3a] border border-yellow-500/25 px-3 py-2.5 text-center">
                                <p class="text-xl font-bold text-yellow-300">{{ $stats['rayon'] }}</p>
                                <p class="text-[11px] uppercase tracking-wide text-white/75">Rayon Aktif</p>
                            </div>
                            <div class="rounded-xl bg-[#0b1f3a] border border-yellow-500/25 px-3 py-2.5 text-center">
                                <p class="text-xl font-bold text-yellow-300">{{ $stats['anggota'] }}</p>
                                <p class="text-[11px] uppercase tracking-wide text-white/75">Kader</p>
                            </div>
                        </div>

                        <div class="grid grid-cols-2 gap-2">
                            <a href="{{ route('gallery.index') }}" class="inline-flex min-h-11 items-center justify-center rounded-xl bg-[#0b1f3a] border border-white/15 px-3 text-sm font-semibold text-white hover:bg-[#0e2748] focus:outline-none focus-visible:ring-2 focus-visible:ring-white/80 transition">
                                Galeri
                            </a>
                        ...
                        <div data-hero-slider-card class="premium-tilt relative rounded-2xl overflow-hidden border border-white/20 flex-1 min-h-0 shadow-xl shadow-black/30" data-premium-tilt>
                            ...
                            <div class="absolute inset-0 flex flex-col justify-between p-5 z-20">
                                ...
                                <div>
                                    ...
                                    <a href="{{ route('posts.berita') }}" data-magnetic class="magnetic-hover relative overflow-hidden px-4 py-2 bg-yellow-500 border border-yellow-400 rounded-full text-[#0f172a] text-sm font-bold hover:bg-yellow-400 transition-all duration-300">
                                        <span class="cta-shimmer absolute inset-0 pointer-events-none"></span>
                                        <span class="relative z-10">Baca Berita</span>
                                    </a>
                                </div>
                            ...
                            @if (count($heroImages) > 1)
                                <div class="absolute bottom-4 right-4 z-20 flex gap-2" role="tablist" aria-label="Slide hero">
                                    @foreach ($heroImages as $index => $heroImage)
                                        <button class="hero-dot w-2 h-2 rounded-full {{ $index === 0 ? 'bg-white' : 'bg-white/40' }} transition-all duration-300" data-slide="{{ $index }}" aria-label="Tampilkan slide {{ $index + 1 }}"></button>
                                    @endforeach
                                </div>
                            @endif
                        </div>
                    ...
                    <div class="hero-desktop-col lg:col-span-7 flex flex-col gap-2 premium-parallax" data-premium-layer="8">
                        ...
                        <div class="bg-white/10 backdrop-blur-sm border border-white/20 rounded-2xl p-4 md:p-4 flex-1 min-h-0 flex flex-col">
                            ...
                            <div class="flex justify-end">
                                <a href="{{ route('about.profil') }}" class="w-9 h-9 rounded-full bg-yellow-500 border border-yellow-400 flex items-center justify-center hover:bg-yellow-400 transition-all duration-300">
