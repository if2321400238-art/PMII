                    ...
                    <div class="lg:col-span-5 flex flex-col gap-2 opacity-0-start animate-fade-in-left">
                        <div class="relative rounded-2xl overflow-hidden border border-white/20 flex-1 min-h-0 h-[360px] sm:h-[420px] md:h-auto">
                                ...
                                <div class="hero-slider absolute inset-0">
                                    @foreach ($heroImages as $index => $heroImage)
                                        <div class="hero-slide absolute inset-0 transition-opacity duration-1000 {{ $index === 0 ? 'opacity-100 z-10' : 'opacity-0 z-0' }}" data-index="{{ $index }}">
                                            <img src="{{ asset('storage/' . $heroImage) }}" alt="Hero Background {{ $index + 1 }}" class="w-full h-full object-cover">
                            ...
                            <div class="absolute inset-0 flex flex-col justify-between p-4 md:p-6 z-20">
                                ...
                                <div>
                                    ...
                                    <a href="{{ route('posts.berita') }}" class="px-4 py-2 bg-yellow-500 border border-yellow-400 rounded-full text-[#0f172a] text-xs md:text-sm font-bold hover:bg-yellow-400 hover:scale-105 transition-all duration-300 shadow-lg animate-pulse-glow">
                                        Baca Berita
                                    </a>
                                </div>
                            </div>
                            ...
                            @if (count($heroImages) > 1)
                                <div class="absolute bottom-4 right-4 z-20 flex gap-2">
                                    @foreach ($heroImages as $index => $heroImage)
                                        <button class="hero-dot w-2 h-2 rounded-full {{ $index === 0 ? 'bg-white' : 'bg-white/40' }} transition-all duration-300" data-slide="{{ $index }}"></button>
                                    @endforeach
                                </div>
                            @endif
                        </div>
                        ...
                        <div class="mt-3 grid grid-cols-2 gap-2 md:hidden">
                            <div class="rounded-xl bg-[#0f172a] border border-yellow-500/30 px-3 py-2 text-center">
                                <div class="text-sm font-bold text-yellow-400">{{ $stats['rayon'] }}</div>
                                <div class="text-[10px] uppercase text-white/70">Rayon</div>
                            </div>
                            <div class="rounded-xl bg-[#0f172a] border border-yellow-500/30 px-3 py-2 text-center">
                                <div class="text-sm font-bold text-yellow-400">{{ $stats['anggota'] }}</div>
